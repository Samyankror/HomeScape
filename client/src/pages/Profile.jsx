...
function Profile() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await fetchWithAuth(
        `/api/user/update/${currUser.user._id}`,
        {
          method: "POST",
          headers: {
  ...
  const handleDeleteUser = async () => {
    try {
      ...
      const data = await fetchWithAuth(
        `/api/user/delete/${currUser.user._id}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleSignOut = async () => {
    try {
      ...
      const res = await fetch("/api/auth/signout", {
        ...
        body: JSON.stringify({ _id: currUser.user._id }),
      });
      const data = await res.json();
      if (!data.success) {
  ...
  const handleShowListing = async () => {
    try {
      ...
      const data = await fetchWithAuth(
        `/api/user/listings/${currUser.user._id}`
      );
  ...
  const handleDeleteListing = async (listingId) => {
    try {
      const data = await fetchWithAuth(`/api/listing/delete/${listingId}`, {
        method: "DELETE",
      });
    ...
    <>
      <div className="p-3 max-w-lg mx-auto mt-7">
        ...
        <form className="flex flex-col gap-4 mt-4" onSubmit={handleSubmit}>
          ...
          <button
            disabled={loading}
            className="bg-slate-700 uppercase p-3 font-semibold text-white rounded-lg
                hover:opacity-90 disabled:opacity-75 cursor-pointer"
          >
              ...
              <div key={listing._id} className="flex gap-4">
                ...
                <div className="flex flex-col  gap-1 items-center ">
                  <button
                    ...
                    className="bg-red-700 text-white uppercase  px-2 py-1 rounded-lg text-xs font-semibold"
                  >
                    Delete
                  </button>
                  <Link to={`/update-listing/${listing._id}`}>
                    <button className="bg-green-700 text-white uppercase px-2 py-1 rounded-lg text-xs font-semibold">
                      Update
                    </button>
                  </Link>
                </div>
